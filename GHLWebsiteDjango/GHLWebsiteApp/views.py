...
def discord_webhook(request):
    if request.method == 'POST':
        ...
        try:
            verify_key = nacl.signing.VerifyKey(bytes.fromhex("1324949615883325450"))
        ...
        # Respond to the PING event from Discord
        if data.get('type') == 1:
        ...
        # Check for a command-style message (like /stats username)
        if "content" in data and data["content"].startswith("/statsskater"):
            # Parse command and argument
            parts = data["content"].split()
            if len(parts) < 2:
                return JsonResponse({"content": "❌ Please specify a username. Example: `/statsskater thebigschnoz`"})

            username = data["content"][len("/statsskater"):].strip()
            player = Player.objects.get(username__iexact=username)
            if not player.exists():
                player = Player.objects.filter(username__icontaines=username)
            if not player.exists():
                return JsonResponse({"content": f"⚠️ Player '{username}' not found."})
            if player.count() > 1:
                matches = ", ".join(player.values_list("username", flat=True)[:5])
                return JsonResponse({
                    "content": f"⚠️ Multiple matches found: {matches}\nPlease be more specific."
                })
            player = player.first()
            ...
            if season is None:
                return JsonResponse({"content": "⚠️ No active season found. Please try again later when Schnoz isn't breaking the website."})            
            ...
            if stats is None:
                return JsonResponse({"content": f"⚠️ No stats found for player '{username}' in the current season."})
            response_message = (
        ...
        elif "content" in data and data["content"].startswith("/statsgoalie"):
            parts = data["content"].split()
            if len(parts) < 2:
                return JsonResponse({"content": "❌ Please specify a username. Example: `/statsgoalie thebigschnoz`"})

            username = data["content"][len("/statsgoalie"):].strip()
            player = Player.objects.get(username__iexact=username)
            if not player.exists():
                player = Player.objects.filter(username__icontaines=username)
            if not player.exists():
