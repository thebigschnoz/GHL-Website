...
def leaders(request):
    ...
    leaders_assists = SkaterRecords.objects.filter(game_num__season_num=seasonSetting).annotate(numassists=Sum("assists")).order_by("-numassists")[:10]
    leaders_points = SkaterRecords.objects.filter(game_num__season_num=seasonSetting).annotate(numpoints=(Sum("goals")+Sum("assists"))).order_by("-numpoints")[:10]
...
def calculate_standings(Season):
    teams = TeamList.objects.filter(isActive=True)
    standings = []
    for team in teams:
        wins = Games.objects.filter(season_num=seasonSetting, a_team_num=team, a_team_gf__gt=F("h_team_gf")).count() + Games.objects.filter(h_team_num=team, h_team_gf__gt=F("a_team_gf")).count()
        losses = Games.objects.filter(season_num=seasonSetting, a_team_num=team, gamelength__lte=3600, a_team_gf__lt=F('h_team_gf')).count() + Games.objects.filter(team2=team, h_team_gf__lt=F('a_team_gf')).count()
        otlosses = Games.objects.filter(season_num=seasonSetting, a_team_num=team, gamelength__gt=3600, a_team_gf=F('h_team_gf')).count() + Games.objects.filter(team2=team, h_team_gf=F('a_team_gf')).count()
